...
/* FIXME: move other DNS_QUERY_TYPES from OVS code here or
 * put this define to OVS source?
 */
...
{
    dns_build_base_answer(dns_answer, in_queryname, query_length, DNS_QUERY_TYPE_A);
...
{
    dns_build_base_answer(dns_answer, in_queryname, query_length, DNS_QUERY_TYPE_AAAA);
...
{
    char *answer, *label_end, *curr_label = (char *)answer_data;
    uint8_t label_len;
    uint16_t answer_length, idx = 0;

    dns_build_base_answer(dns_answer, in_queryname, query_length, DNS_QUERY_TYPE_PTR);

    /* Initialize string 2 chars longer than real answer:
     * first label length and terminating zero-length label. */
    answer_length = strlen(answer_data) + 2;
    answer = (char *)xmalloc(answer_length);

    while ((label_len = strlen(curr_label))) {
        label_end = strchr(curr_label, '.');
        if (label_end) {
            label_len = label_end - curr_label;
        }
        *(answer + idx++) = label_len;
        strncpy(answer + idx, curr_label, label_len);
        curr_label += (label_len + 1);
        idx += label_len;
    };
    *(answer + idx++) = 0;


    VLOG_DBG("answer: %s, answer_length: %s, idx: %d",
             *answer, answer_length, idx);
    put_be16(dns_answer, htons(idx));
    ofpbuf_put(dns_answer, answer, idx);
    free(answer);
}
    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    if (!(query_type == DNS_QUERY_TYPE_A || query_type == DNS_QUERY_TYPE_AAAA
          || query_type == DNS_QUERY_TYPE_ANY || query_type == DNS_QUERY_TYPE_PTR)) {
        ds_destroy(&query_name);
        goto exit;
    }
    ...
    VLOG_DBG("in_queryname: %s, query_name: %s, answer_data %s",
             in_queryname, query_name.string, answer_data);
    ...
    if (query_type == DNS_QUERY_TYPE_PTR) {
        ...
        if (query_type == DNS_QUERY_TYPE_A || query_type == DNS_QUERY_TYPE_ANY) {
            for (size_t i = 0; i < ip_addrs.n_ipv4_addrs; i++) {
                dns_build_a_answer(&dns_answer, in_queryname, idx, ip_addrs.ipv4_addrs[i].addr);
                ancount++;
            }
        }

        if (query_type == DNS_QUERY_TYPE_AAAA || query_type == DNS_QUERY_TYPE_ANY) {
            for (size_t i = 0; i < ip_addrs.n_ipv6_addrs; i++) {
                dns_build_aaaa_answer(&dns_answer, in_queryname, idx, &ip_addrs.ipv6_addrs[i].addr);
                ancount++;
            }
        }
